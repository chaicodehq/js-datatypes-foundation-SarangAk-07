...
export function findPassenger(passengers, name) {
  ...
  return passengers.find((p)=>typeof p.name =="string" && p.name.toLowerCase() === lowerName);
...
export function getPassengerIndex(passengers, name) {
  ...
  let lowerName = name.toLowerCase();
  return passengers.findIndex((p)=> typeof p.name == "string" && p.name.toLowerCase()==lowerName);
}
