...
export function repeatPattern(pattern, times) {
  if(typeof pattern !== "string" || typeof times !== "number" || !Number.isInteger(times) || times <= 0){
    return ""
  }
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  if(typeof colorString !== "string" || typeof oldSep !== "string" ||
    typeof newSep !== "string"){
    return ""
  }
  const part = colorString.split(oldSep)
  const result = part.join(newSep)
  return result
}
